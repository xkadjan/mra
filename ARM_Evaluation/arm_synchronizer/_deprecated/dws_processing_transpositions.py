...
def get_seconds(utc):
    seconds = (utc[0].astype('int')*3600 + utc[1].astype('int')*60 + utc[2].astype('float')) #+ (micros/1000000)
    return seconds
#
#def get_log_time(dewesoft):
#    log_time = round(dewesoft.utc_time[0]) + dewesoft["index"]
#    return log_time

def get_coordinate_dws(coor):
    coor = coor/60 # tak pry pouze takto dle Petra P.
#    coor = round(coor/60) + (coor%60)/60 # dewesoft udava hodnoty lat a lon v minutach
#    coor = coor # ne vecheny exporty dewesoft udava hodnoty lat a lon v minutach
    return coor
