...
def configArgParser():
    ...
    parser.add('-r','--rate', type=int, help='MRA rate in Hz (for static measurement)')
    parser.add('-s','--static', type=lambda x: (str(x).lower() == 'true'), help='evaluate only static measurement')
    parser.add('-sp','--static_period', type=int, help='static measurement period in [s]')
    args = parser.parse()
    return args
...
if args.new_preproccess:
    ...
    # static = True
    # if args.static:
    #     rtk.drop_points_wo_arm(arm.static)
    #     rtk.drop_points_wo_rtk(arm.static)
    # else:
...
if args.static:
    pltr.plot_rtk(arm.static,'MRA',"r",0)
else:
    pltr.plot_rtk(arm.arm_20hz,'MRA',"r",0)
pltr.plot_rtk(rtk.novatel,args.rtk_names[0],"g",20)
pltr.plot_rtk(rtk.tersus,args.rtk_names[1],"y",40)
pltr.plot_rtk(rtk.ashtech,args.rtk_names[2],"b",60)
pltr.plot_rtk(rtk.ublox,args.rtk_names[3],"m",80)
pltr.print_to_file()
